...
async function run() {
  const phase1 = (await db.select().from(roadmapProjects)).find(
    (p) => p.title === 'Phase 1 - Foundation, Registry, and Core Admin',
  );

  if (!phase1) {
    throw new Error('Phase 1 project not found');
  }
  ...
  console.log('Phase 1 summary:', summary);
}
