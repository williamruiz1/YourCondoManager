...
> **Authority note (2026-07-19):** The generated JSON files currently contain April 2026 Admin Roadmap snapshots. They are useful for repository structure, but they are not current product-status authority. Use live PocketPM for task and product lifecycle, and use `docs/projects/platform-overhaul/00-index.md` for the reconciled Platform Overhaul sequence. Do not hand-edit the generated JSON.
...
The generator requires a valid `DATABASE_URL` because it reads the legacy Admin Roadmap tables. If that connection is unavailable, leave the JSON unchanged and record the failed refresh in the relevant reconciliation document.
