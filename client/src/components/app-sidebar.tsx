...
function ZoneGroup({ zone, location }: ZoneGroupProps) {
  ...
  return (
    <SidebarGroup>
      <SidebarGroupLabel className="label-caps text-sidebar-foreground/70">
        {zone.label}
      </SidebarGroupLabel>
      <SidebarGroupContent>
        <SidebarMenu>
                ...
                {zone.items.map((item, index) => {
                  ...
                  return (
                    <Fragment key={item.url}>
                      {showGroupHeader ? (
                        <li
                          className="px-2 pb-0.5 pt-2 first:pt-0 text-[10px] font-label font-semibold uppercase tracking-widest text-sidebar-foreground/70 select-none group-data-[collapsible=icon]:hidden"
      ...
      <SidebarHeader className="border-b border-slate-200/50 dark:border-slate-800/50">
        ...
        >
          ...
          <div className="group-data-[collapsible=icon]:hidden">
            ...
            <p className="text-[10px] text-sidebar-foreground/70 uppercase tracking-widest leading-tight font-label">
              Property Operations
            </p>
          </div>
        </Link>
        ...
        <div className="px-3 pb-3 group-data-[collapsible=icon]:hidden">
          {activeAssociationId ? (
            ...
            >
              <span className="material-symbols-outlined text-[16px] text-sidebar-foreground/70 flex-shrink-0">
                domain
              </span>
              <div className="min-w-0">
                <div className="text-[10px] uppercase tracking-wide text-sidebar-foreground/70 leading-none mb-0.5 font-label">
                  Association
                </div>
                <div
              ...
              {visibleFooterItems.map((item) => {
                ...
                return (
                  <Link key={item.url} href={item.url}>
                    <button
                      className="flex items-center gap-1.5 text-xs text-sidebar-foreground/70 hover:text-sidebar-foreground transition-colors font-body"
