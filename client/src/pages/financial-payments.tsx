...
function AutopayTab({ associationId }: { associationId: string | null }) {
  const { toast } = useToast();
  const [statusFilter, setStatusFilter] = useState<string>("active");
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [runs, setRuns] = useState<Record<string, AutopayRun[]>>({});
  ...
  const triggerRun = useMutation({
    ...
    onSuccess: (data) => {
      toast({ title: `Collection run complete`, description: `${data.succeeded} succeeded, ${data.failed} failed of ${data.totalDue} due` });
      refetch();
    },
  ...
  const updateEnrollment = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: "active" | "paused" | "cancelled" }) => {
      const res = await apiRequest("PATCH", `/api/financial/autopay/enrollments/${id}`, { status });
  ...
  async function loadRuns(enrollmentId: string) {
    if (runs[enrollmentId]) return;
    try {
      const res = await apiRequest("GET", `/api/financial/autopay/enrollments/${enrollmentId}/runs`);
      if (res.ok) {
        const data = await res.json();
        setRuns(prev => ({ ...prev, [enrollmentId]: data }));
      }
    } catch {}
  }
  ...
  const filtered = enrollments.filter(r =>
    statusFilter === "all" ? true : r.enrollment.status === statusFilter
  );

  if (!associationId) {
    return (
      <Card>
        <CardContent className="pt-6 text-sm text-muted-foreground">Select an association to view autopay enrollments.</CardContent>
      </Card>
    );
  }
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <CardTitle className="text-base">Autopay Enrollments</CardTitle>
              <CardDescription>Owners enrolled in recurring automatic payments</CardDescription>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {triggerRun.isPending ? "Running…" : "Run Collections Now"}
              </Button>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-6 text-sm text-muted-foreground">Loading enrollments…</div>
          ) : filtered.length === 0 ? (
            <div className="p-6 text-sm text-muted-foreground">No enrollments found for the selected filter.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {filtered.map(({ enrollment, unit, person }) => (
                    ...
                    <TableRow className="cursor-pointer hover:bg-muted/30">
                      ...
                      <TableCell className="text-sm font-medium">
                        {unit ? (
                          ...
                          <span className="text-muted-foreground text-xs">{enrollment.unitId.slice(0, 8)}</span>
                        )}
                      </TableCell>
                      <TableCell className="text-sm">
                        {person ? (
                          <div>
                            <div>{[person.firstName, person.lastName].filter(Boolean).join(" ") || "—"}</div>
                            {person.email && <div className="text-xs text-muted-foreground">{person.email}</div>}
                          </div>
                        ) : (
                          <span className="text-muted-foreground text-xs">{enrollment.personId.slice(0, 8)}</span>
                        )}
                      </TableCell>
                      <TableCell className="text-sm capitalize">{enrollment.frequency}</TableCell>
                      <TableCell className="text-sm">${Number(enrollment.amount).toFixed(2)}</TableCell>
                      ...
                      <TableCell>
                        <Badge variant={STATUS_BADGE[enrollment.status]?.variant ?? "outline"} className="text-xs">
                        ...
                        <div className="flex gap-1">
                          {enrollment.status === "active" && (
                            <Button
                              ...
                              onClick={() => updateEnrollment.mutate({ id: enrollment.id, status: "paused" })}
                          ...
                          {enrollment.status === "paused" && (
                            <Button
                              ...
                              variant="outline"
                              className="h-7 text-xs"
                              onClick={() => updateEnrollment.mutate({ id: enrollment.id, status: "active" })}
                              disabled={updateEnrollment.isPending}
                            >
                              Resume
                            </Button>
                          ...
                          {enrollment.status !== "cancelled" && (
                            <Button
                              ...
                              onClick={() => updateEnrollment.mutate({ id: enrollment.id, status: "cancelled" })}
                              disabled={updateEnrollment.isPending}
                            >
                              Cancel
                            </Button>
                    ...
                    {expandedId === enrollment.id && (
                      <TableRow>
                        <TableCell colSpan={8} className="bg-muted/20 px-6 py-3">
                          ...
                          ) : runs[enrollment.id].length === 0 ? (
                            <div className="text-xs text-muted-foreground">No runs yet</div>
                          ) : (
                            <div className="space-y-1">
                              {runs[enrollment.id].map((run) => (
                                <div key={run.id} className="flex items-center gap-3 text-xs">
                                  <AutopayRunBadge status={run.status} />
    ...
    <div className="flex flex-col min-h-0">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="autopay" className="mt-4">
          <AutopayTab associationId={selectedAssociationId} />
        </TabsContent>
