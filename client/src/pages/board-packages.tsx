...
export function BoardPackagesContent() {
  ...
  type PortalAccess = { id: string; email: string; role: string; associationId: string };
  const { data: portalAccesses = [] } = useQuery<PortalAccess[]>({
    queryKey: ["/api/portal/access"],
  });
  const boardMemberEmails = useMemo(() => {
    if (!activeAssociationId) return [];
    return portalAccesses
      .filter((a) => a.associationId === activeAssociationId && a.role === "board-member" && a.email)
      .map((a) => a.email);
  }, [portalAccesses, activeAssociationId]);
  const { data: templates = [] } = useQuery<BoardPackageTemplate[]>({
    queryKey: [templatesQueryKey],
    enabled: Boolean(activeAssociationId),
  });
  const { data: packages = [], refetch: refetchPackages } = useQuery<BoardPackage[]>({
    queryKey: [packagesQueryKey],
    enabled: Boolean(activeAssociationId),
  });

  type DistributionHistoryItem = { id: string; title: string; periodLabel: string; distributedAt: string; distributedBy: string | null; status: string };
  type DistributionHistoryData = {
    distributed: DistributionHistoryItem[];
    autoScheduleStats: { id: string; title: string; autoGenerate: number; lastAutoGeneratedAt: string | null; generateDaysBefore: number; frequency: string }[];
  };
  const { data: distributionHistory, refetch: refetchHistory } = useQuery<DistributionHistoryData>({
    queryKey: ["/api/admin/board-packages/distribution-history", activeAssociationId],
    queryFn: async () => {
      if (!activeAssociationId) return { distributed: [], autoScheduleStats: [] };
      const res = await apiRequest("GET", `/api/admin/board-packages/distribution-history?associationId=${activeAssociationId}`);
      return res.json();
    },
    enabled: Boolean(activeAssociationId),
  });
  ...
  return (
    <>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3 md:hidden">
                  ...
                  <div key={template.id} className="rounded-xl border p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">{getAssociationName(template.associationId)}</div>
                      </div>
                      <Badge variant="outline">{template.frequency}</Badge>
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {template.autoGenerate
                        ? `${template.generateDaysBefore} day(s) before ${template.meetingType || "meeting"} · ${template.lastAutoGeneratedAt ? `Last run ${new Date(template.lastAutoGeneratedAt).toLocaleString()}` : "Not yet run"}`
                        : "Manual only"}
                    </div>
              ...
              {templates.length === 0 ? (
                <div className="rounded-xl border border-dashed p-4 text-center text-sm text-muted-foreground">No board package templates yet.</div>
              ) : null}
            </div>
          </CardContent>
        </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-md border p-4 space-y-3">
                  ...
                  <div className={`flex justify-end ${isMobile ? "flex-col" : ""}`}>
                    ...
                    >
                      {distributePackage.isPending ? "Sending…" : "Send Approved Package"}
                    </Button>
                  </div>
                  {lastDistributionResult && lastDistributionResult.packageId === selectedPackage.id && (
                    <div className="rounded-md border bg-muted/30 p-3 space-y-2 text-sm">
                      <div className="font-medium text-xs uppercase tracking-wide text-muted-foreground">Last Delivery</div>
                      <div className="text-xs text-muted-foreground">
                        {new Date(lastDistributionResult.deliveredAt).toLocaleString()}
                      </div>
                      ...
                      {lastDistributionResult.failedCount > 0 && (
                        <div className="text-xs text-destructive">
                          {lastDistributionResult.failedCount} delivery failure(s) — check email provider configuration.
                        </div>
                      )}
                    </div>
                  )}
      ...
      {/* Automation Status & Distribution History */}
      {activeAssociationId && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Automation &amp; Distribution History</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
                ...
                <div className={isMobile ? "space-y-2" : "flex flex-wrap gap-2"}>
                  {distributionHistory.autoScheduleStats.map((t) => (
                    <div key={t.id} className={`rounded-md border px-3 py-2 text-sm ${isMobile ? "w-full" : ""} ${t.autoGenerate ? "bg-green-50 dark:bg-green-950/20 border-green-300" : "bg-muted"}`}>
                      ...
                      {t.autoGenerate ? (
                        <span className="ml-2 text-xs text-green-700 dark:text-green-400">auto — {t.generateDaysBefore}d before meeting{t.lastAutoGeneratedAt ? `, last run ${new Date(t.lastAutoGeneratedAt).toLocaleDateString()}` : ", never run"}</span>
                      ) : (
            ...
            <div className="space-y-2">
              <div className="text-sm font-medium">Distribution Log ({distributionHistory?.distributed.length ?? 0} packages sent)</div>
              {distributionHistory?.distributed.length === 0 ? (
                <div className="text-sm text-muted-foreground">No packages have been distributed yet for this association.</div>
              ) : (
                <>
                  <div className="hidden md:block">
                    <Table>
                      ...
                      <TableBody>
                        {(distributionHistory?.distributed ?? []).slice().reverse().map((d) => (
                          <TableRow key={d.id}>
                            <TableCell className="font-medium">{d.title}</TableCell>
                            <TableCell>{d.periodLabel}</TableCell>
                            <TableCell className="text-sm text-muted-foreground">{new Date(d.distributedAt).toLocaleString()}</TableCell>
                            <TableCell className="text-sm text-muted-foreground">{d.distributedBy ?? "—"}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                  ...
                  <div className="space-y-3 md:hidden">
                    {(distributionHistory?.distributed ?? []).slice().reverse().map((d) => (
                      <div key={d.id} className="rounded-xl border p-4 space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">{d.periodLabel}</div>
                        <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
                          <span>{new Date(d.distributedAt).toLocaleString()}</span>
                          <span>{d.distributedBy ?? "—"}</span>
                        </div>
                      </div>
                    ))}
                  </div>
